...
import { dashboardkpis } from './dashboard-kpis-data1'
import './dashboard-kpis.css'
export default function Dashboardkpis1() {
  return (
    <div className='row'>
        {dashboardkpis.map(dashboardkpis =>
          <div className='col-12 col-sm-6 col-md-4 col-lg-2 mt-2' key={dashboardkpis.title}>
            <a tabIndex={0} href='#' title={`${dashboardkpis.title} ${dashboardkpis.count}`} className="shadow card p-3 border-radius3 w-100 d-flex justify-content-between align-items-center card-body h-100 gap-1" status-kpis={dashboardkpis.kpicode}>
